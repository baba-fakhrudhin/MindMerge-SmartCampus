...
<div class="dashboard-card"><div class="card-icon purple"><i class="fa-solid fa-graduation-cap"></i></div><h3>Latest GPA</h3><h1><?php echo $stats['latest_gpa'] !== null ? $stats['latest_gpa'] : '—'; ?></h1></div>
<div class="dashboard-card"><div class="card-icon orange"><i class="fa-solid fa-file-lines"></i></div><h3>Upcoming Exams</h3><h1><?php echo $stats['upcoming_exams']; ?></h1></div>
...
<div class="portal-chart-card"><h3>Academic Performance</h3><canvas id="resultsTrendChart"></canvas></div>
</div>
<div class="dashboard-section">
...
const resultsTrend = <?php echo json_encode($stats['results_trend']); ?>;
if (resultsTrend.labels && resultsTrend.labels.length) {
new Chart(document.getElementById('resultsTrendChart'), { type:'line', data:{ labels:resultsTrend.labels, datasets:[{ data:resultsTrend.values, borderColor:'#7c3aed', spanGaps:true, tension:0.3 }] }, options:{ responsive:true, maintainAspectRatio:false, scales:{ y:{ min:0, max:4 } } } });
}
</script>
</body>
</html>
